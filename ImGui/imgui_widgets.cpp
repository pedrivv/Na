...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    static std::unordered_map<std::string, bool> valuesMapBool;

    if (pressed)
    {
        auto valuesac = valuesMapBool.find(label);
        if (valuesac == valuesMapBool.end())
        {
            valuesMapBool.insert({ label, true });
        }
        else
        {
            valuesMapBool.find(label)->second = true;
        }
    }
    ...
    it_anim->second.a = ImLerp(it_anim->second.a, valuesMapBool.find(label)->second ? size.x : 0.f, g.IO.DeltaTime * 10.f);
    ...
    ImGui::PushClipRect(bb.Min, bb.Max, true);
    {
        ...
        {
            valuesMapBool.find(label)->second = false;
        }
...
bool ImGui::TabHeaderButton(const char* ButtonName, bool enabled, int direction)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImColor BGColor = enabled ? ImColor(232, 63, 212) : ImColor(16, 16, 16);
    ImColor TextColor = enabled ? ImColor(255, 255, 255, 255) : ImColor(1.f, 1.f, 1.f, 0.4f);

    if (direction == 0)
    {
        window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), BGColor, 5, ImDrawFlags_RoundCornersAll);
        window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(ImGuiCol_Border), 5, ImDrawFlags_RoundCornersAll);
    }
    else if (direction == 1)
    {
        window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), BGColor, 5, ImDrawFlags_RoundCornersLeft);
        window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(ImGuiCol_Border), 5, ImDrawFlags_RoundCornersLeft);
    }
    else if (direction == 2)
    {
        window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), BGColor);
        window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(ImGuiCol_Border));
    }
    else if (direction == 3)
    {
        window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), BGColor, 5, ImDrawFlags_RoundCornersRight);
        window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(ImGuiCol_Border), 5, ImDrawFlags_RoundCornersRight);
    }
...
bool ImGui::TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (int i = 0; i < Tabs.size(); i++)
    {
        if (Tabs.size() != 1)
        {
            if (i == 0)
            {
                if (TabHeaderButton(Tabs.at(i), *currentTab == i, 1))
                    *currentTab = i;
            }
            else if (i == Tabs.size() - 1)
            {
                ImGui::SameLine(0, 0);
                if (TabHeaderButton(Tabs.at(i), *currentTab == i, 3))
                    *currentTab = i;
            }
            else
            {
                ImGui::SameLine(0, 0);
                if (TabHeaderButton(Tabs.at(i), *currentTab == i, 2))
                    *currentTab = i;
            }
        }
        else
        {
            if (TabHeaderButton(Tabs.at(i), *currentTab == i, 0))
                *currentTab = i;
        }
    }
...
std::unordered_map <std::string, float> allcombopibonui;
std::unordered_map <std::string, int> allcombopibonuisexo2;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    else
    {
        ...
        auto animatorsec = allcombopibonui.find(std::string(label).append("designpiroca"));
        if (animatorsec == allcombopibonui.end())
        {
            allcombopibonui.insert({ std::string(label).append("designpiroca") , CalcMaxPopupHeightFromItemCount(allcombopibonuisexo2.find(label)->second) * it_anim->second.open_anim });
        }
        else
        {
            animatorsec->second = CalcMaxPopupHeightFromItemCount(allcombopibonuisexo2.find(label)->second) * it_anim->second.open_anim;
        }
    }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(InterRegular14, 14, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(InterRegular14, 14, draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
void ImGui::MultiCombo(std::string name, std::vector<int>& variable, const char* labels[], int count, std::string& preview)
{
    ...
    auto sexo = allcombopibonuisexo2.find(name);
    if (sexo == allcombopibonuisexo2.end())
    {
        allcombopibonuisexo2.insert({ name, count });
    }
    else
        allcombopibonuisexo2.find(name)->second = count;

    if (ImGui::BeginCombo(name.c_str(), (howmuchsel(variable) > 0 ? std::to_string(howmuchsel(variable)) + " selected" : "None").c_str())) // draw start
    {
